...
void Filtre(Personne (*personne)[], int nbrow, int choix)
{
	...
	for (ligne = 0; ligne <= nbrow; ligne++)
	{
		...
		do
		{
			if (OriganisationCSV(&(*personne)[ligne], choix)[colonne] == cherchant[colonne])
			{
				colonne++;
			}
			else programme = 0;
		} while (cherchant[colonne] != '\n' && programme == 1);
		if (programme == 1)
		{
			printf("-------------------------------------------------------------");
			printf("\n%s\n", (*personne)[ligne].prenom);
			printf("%s\n", (*personne)[ligne].nom);
			printf("%s\n", (*personne)[ligne].ville);
			printf("%s\n", (*personne)[ligne].codepostal);
			printf("%s\n", (*personne)[ligne].tel);
			printf("%s\n", (*personne)[ligne].email);
			printf("%s\n", (*personne)[ligne].profession);
			printf("-------------------------------------------------------------");
		}
	}
}
...
int main()
{
	...
	while (programme == 1)
	{
		...
		case 1:
			...
			case 8:
				Clear();
				Filtre(pt_info, nbrow, reponse - 2);
				break;
			//Retour
